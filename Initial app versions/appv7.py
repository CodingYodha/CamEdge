...
def process_single_frame(frame, model_instance, selected_class, transfer_zone_coords, conf_threshold, 
                        ...
                        loaded_count, unloaded_count, show_zone_border=False):
    annotated_frame = frame.copy()
    tx, ty, tw, th = transfer_zone_coords
    TRANSFER_ZONE_COORDS_x1y1x2y2 = (tx, ty, tx + tw, ty + th)

    if show_zone_border and tx >= 0 and ty >= 0 and tw > 0 and th > 0 : 
    ...
    if hasattr(model_instance, 'model') and hasattr(model_instance.model, 'names') and model_instance.model.names:
        ...
        # else: st.warning(f"Selected class '{selected_class}' not found in model. Tracking all/first.") # Can be noisy
    # else: st.error("Model class names not accessible. Tracking all.") # Can be noisy
    ...
    try:
        ...
        # st.error(f"Error during YOLO tracking: {e}") # Can be very noisy during processing
    ...
    try:
        ...
        while True:
            ...
            annotated_frame, current_loaded, current_unloaded, current_next_id, items_in_frame = process_single_frame(
                ...
                item_zone_tracking_info, loaded_count, unloaded_count, show_zone_border=False)
        ...
        st.error(f"Error processing video: {str(e)}") # Keep this error for critical failures
        ...
        # Set session state path AFTER checks
...
def live_stream_processing_loop(selected_class, transfer_zone_rect, conf_threshold, camera_index):
    ...
    if not cap.isOpened(): st.error(f"Could not open camera {camera_index}."); st.session_state.live_stream_active = False; return
    item_id_map, next_display_item_id, item_zone_tracking_info, loaded_count, unloaded_count = {}, [1], {}, [0], [0]
    stream_placeholder = st.empty()
    live_metrics_col1, live_metrics_col2, live_metrics_col3 = st.columns(3)
    unique_ph, loaded_ph, unloaded_ph = live_metrics_col1.empty(), live_metrics_col2.empty(), live_metrics_col3.empty()
    try:
        while st.session_state.get('live_stream_active', False): 
            ret, frame = cap.read()
            if not ret or frame is None: st.warning("Failed to read frame."); st.session_state.live_stream_active = False; break
            annotated_frame, cur_loaded, cur_unloaded, cur_next_id, _ = process_single_frame(
                frame, model, selected_class, transfer_zone_rect, conf_threshold, item_id_map, next_display_item_id, 
                item_zone_tracking_info, loaded_count, unloaded_count, show_zone_border=False)
            ...
            unique_ph.metric("Unique Items", total_unique); loaded_ph.metric("Loaded", cur_loaded); unloaded_ph.metric("Unloaded", cur_unloaded)
    except Exception as e: st.error(f"Error in live stream: {str(e)}")
    finally:
        if cap.isOpened(): cap.release()
        stream_placeholder.empty(); unique_ph.empty(); loaded_ph.empty(); unloaded_ph.empty()
...
default_states = {
    ...
    'final_loaded_count': 0, # For Upload Video results
    'final_unloaded_count': 0, # For Upload Video results
    'final_unique_count': 0, # For Upload Video results
    'processing_status_message': "", # For Upload Video status
    'is_processing': False # For Upload Video status
}
...
if st.session_state.processing_mode != new_processing_mode: 
    ...
    if new_processing_mode == "Upload Video":
        ...
        st.session_state.processing_status_message = ""; st.session_state.is_processing = False # Reset upload status
    st.rerun()
...
if st.session_state.processing_mode == "Upload Video":
    ...
    if uploaded_file is not None:
        if st.session_state.uploaded_file_name != uploaded_file.name:
            ...
            # Reset previous processing results when a new file is selected
            ...
            st.rerun() # Rerun to clear UI elements based on new state
    else: 
    ...
    with col1:
        ...
        if uploaded_file is not None and st.session_state.first_frame_roi is None and not st.session_state.is_processing: # Avoid during processing
    ...
    with col2:
        ...
        if st.session_state.is_processing:
            ...
            # Live metrics will be updated by the loop directly
            ...
            loaded_metric_ph.metric("Total Loaded", st.session_state.final_loaded_count) # Show 0 or previous if not reset
            unloaded_metric_ph.metric("Total Unloaded", st.session_state.final_unloaded_count)
            unique_metric_ph.metric("Total Unique Items", st.session_state.final_unique_count)
            if st.session_state.processing_status_message: # Show if there's any non-empty non-processing message
                 status_message_placeholder.info(st.session_state.processing_status_message)
            else:
                 status_message_placeholder.empty()
            video_display_placeholder.empty()
        ...
        if st.button(f"🚀 Process Video for {st.session_state.selected_class}s", use_container_width=True, key="process_video_btn", disabled=process_btn_disabled):
            ...
            st.session_state.processed_video_path = None 
            st.session_state.final_loaded_count = 0 # Reset for new processing
            st.session_state.final_unloaded_count = 0
            st.session_state.final_unique_count = 0
            video_display_placeholder.empty()
            status_message_placeholder.info(st.session_state.processing_status_message) # Show processing status
            ...
            for frame_data in process_video_streamlit(video_bytes, st.session_state.selected_class, transfer_zone, conf_threshold_st):
                annotated_frame, loaded, unloaded, unique, progress, temp_vid_path = frame_data
                final_results_from_gen = frame_data # Store last yielded result
            ...
            if final_results_from_gen:
                ...
                else: # Handle if video saving failed within generator
                    st.session_state.processed_video_path = None
                    if fin_path is None and final_results_from_gen[0] is None : # check if generator indicated error by yielding None for frame
    ...
    with col1_live:
        ...
        if model is None: st.error("Model not loaded. Live stream disabled.")
            ...
            if st.session_state.live_stream_state == "initial":
                st.info("Setup camera and ROI before starting.")
                ...
                if st.button("📸 Capture Frame for ROI", key="cap_frame_btn", use_container_width=True):
                    with st.spinner(f"Accessing camera {cam_idx}..."):
                        cap_roi = cv2.VideoCapture(cam_idx)
                        if cap_roi.isOpened():
                            ...
                            else: st.error(f"Failed to capture from camera {cam_idx}.")
                        else: st.error(f"Could not open camera {cam_idx}.")
            elif st.session_state.live_stream_state == "roi_setup":
                st.info("Adjust ROI for live stream.")
                if st.session_state.live_stream_captured_frame is not None:
                    ...
                    sel_cls_live_idx = AVAILABLE_CLASSES.index(st.session_state.get('live_selected_class')) if st.session_state.get('live_selected_class') in AVAILABLE_CLASSES else 0
                    ...
                    if st.button("🚀 Start Live Detection",key="start_live_det",use_container_width=True):
                ...
                st.info("Live stream active. ROI not shown.")
                st.markdown(f"**Tracking:** `{st.session_state.get('live_selected_class','N/A')}` **Conf:** `{st.session_state.get('live_conf_thresh','N/A')}`")
                st.markdown(f"**ROI:** `{st.session_state.get('live_stream_roi_coords')}` (Not on stream)")
    ...
    with col2_live:
        ...
        if st.session_state.live_stream_state == "roi_setup" and st.session_state.live_stream_captured_frame is not None:
            ...
            if model: live_stream_processing_loop(st.session_state.live_selected_class, st.session_state.live_stream_roi_coords, st.session_state.live_conf_thresh, st.session_state.live_camera_idx)
...
if st.sidebar.button("📷 Camera Test"):
    ...
    try:
        ...
        cap_t = cv2.VideoCapture(cam_idx_t) 
        if cap_t.isOpened(): st.sidebar.success(f"✅ Camera ({cam_idx_t}) OK!")
        else: st.sidebar.error(f"❌ No camera ({cam_idx_t}).")
    except Exception as e_ct: st.sidebar.error(f"❌ Test failed: {e_ct}")
    finally:
        if cap_t and cap_t.isOpened(): cap_t.release()
with st.sidebar.expander("❓ Help & Tips"):
    st.markdown("""**ROI Tips:** Purple box at setup. Not on live/processed frames. Thin border on final thumbnail.\n\n**Live Stream:** Capture frame for ROI. Good light. Test camera.\n\n**General:** Adjust confidence. Check `best_bag_box.pt`.""")
if st.sidebar.checkbox("🐛 Debug Mode"):
    st.sidebar.markdown("### Debug Information")
    debug_data = {k: (f"Shape: {v.shape}" if isinstance(v, np.ndarray) else v) for k,v in st.session_state.items()}
    st.sidebar.json(debug_data, expanded=False)