...
def process_single_frame(frame, model_instance, selected_class, transfer_zone_coords, conf_threshold, 
                        ...
                        loaded_count, unloaded_count, show_roi_on_video_frames=False): # Parameter name changed
    annotated_frame = frame.copy()
    tx, ty, tw, th = transfer_zone_coords
    TRANSFER_ZONE_COORDS_x1y1x2y2 = (tx, ty, tx + tw, ty + th)

    # Draw semi-transparent ROI on video frames if toggled ON
    if show_roi_on_video_frames and tx >= 0 and ty >= 0 and tw > 0 and th > 0:
        overlay = annotated_frame.copy()
        cv2.rectangle(overlay, (tx, ty), (tx + tw, ty + th), (255, 0, 255), -1)  # Purple, filled
        alpha = 0.3  # Transparency factor
        cv2.addWeighted(overlay, alpha, annotated_frame, 1 - alpha, 0, annotated_frame)
        cv2.rectangle(annotated_frame, (tx, ty), (tx + tw, ty + th), (255, 0, 255), 2)  # Purple, border
...
def process_video_streamlit(video_bytes, selected_class, transfer_zone_rect, conf_threshold, show_roi_on_video_toggle_val): # Added show_roi_on_video_toggle_val
    if model is None:
    ...
    try:
        ...
        while True:
            ...
            annotated_frame, current_loaded, current_unloaded, current_next_id, items_in_frame = process_single_frame(
                ...
                show_roi_on_video_frames=show_roi_on_video_toggle_val) # Pass toggle state
    ...
    try:
        while st.session_state.get('live_stream_active', False): 
            ...
            if not ret or frame is None: 
                st.warning("Failed to read frame from camera. Stream may have ended.")
                st.session_state.live_stream_active = False 
                break
            ...
            annotated_frame, current_total_loaded, current_total_unloaded, current_next_id, items_in_frame = process_single_frame(
                ...
                item_id_map, next_display_item_id, item_zone_tracking_info, 
                loaded_count, unloaded_count, show_roi_on_video_frames=False # Explicitly False for live stream
            )
...
default_states = {
    ...
    'is_processing': False,
    'show_roi_on_video_toggle_upload': False # Added for the new toggle default
}
...
if st.session_state.processing_mode == "Upload Video":
    ...
    with col1:
        ...
        st.toggle("Show ROI on Processed Video", 
                         ...
                         value=st.session_state.show_roi_on_video_toggle_upload, # Use session state for persistence
    ...
    with col2:
                ...
                try:
                    ...
                    if cap_thumb.isOpened():
                        ...
                        if ret_thumb:
                            ...
                            # Thumbnail always shows thin border, not affected by the new toggle
        ...
        if st.button(f"🚀 Process Video for {st.session_state.selected_class}s", use_container_width=True, key="process_video_btn", disabled=process_btn_disabled):
            ...
            # Get the toggle state from session_state using its key
            ...
            for frame_data in process_video_streamlit(video_bytes, st.session_state.selected_class, 
                                                      ...
                                                      show_roi_on_frames_value): # Pass the toggle state
            ...
            if final_results_from_gen:
                ...
                if fin_path and os.path.exists(fin_path):
                    ...
                    st.session_state.processed_video_path = None
                    if fin_path is None and (final_results_from_gen[0] is None and final_results_from_gen[1] is None): # Check if processing truly failed
    ...
    with col1_live:
        ...
        if model is None: st.error("Model not loaded. Live stream disabled.")
            ...
            if st.session_state.live_stream_state == "initial":
                ...
                st.info("Live stream active. ROI not shown on stream.")
...
with st.sidebar.expander("❓ Help & Tips"):
    st.markdown("""**ROI Tips:** Purple box at setup. Not on live/processed frames by default (toggle for processed video). Thin border on final thumbnail.\n\n**Live Stream:** Capture frame for ROI. Good light. Test camera. ROI not shown on live feed.\n\n**General:** Adjust confidence. Ensure `best_bag_box.pt` is present.""")
if st.sidebar.checkbox("🐛 Debug Mode"):
    st.sidebar.markdown("### Debug Information")
    # Filter out large image arrays for cleaner debug output
    debug_data_filtered = {}
    for k, v in st.session_state.items():
        if isinstance(v, np.ndarray) and v.ndim >= 2: # Likely an image
             debug_data_filtered[k] = f"Numpy array (shape: {v.shape}, dtype: {v.dtype})"
        elif isinstance(v, bytes) and len(v) > 1024: # Large byte string
             debug_data_filtered[k] = f"Bytes (length: {len(v)})"
        else:
