...
def draw_roi_preview(frame, roi_coords, show_transparent=False):
    ...
    if not show_transparent: # This is for ROI setup phase - purple box
    ...
    # This 'elif' is for the thumbnail after processing, or if explicitly needed elsewhere with a border
...
def process_single_frame(frame, model_instance, selected_class, transfer_zone_coords, conf_threshold, 
                        ...
                        loaded_count, unloaded_count, show_zone_border=False): # DEFAULT CHANGED
    ...
    if hasattr(model_instance, 'model') and hasattr(model_instance.model, 'names') and model_instance.model.names:
        ...
        if selected_class in class_names_list:
            ...
            st.warning(f"Selected class '{selected_class}' not found in model. Tracking all classes or first if available.")
    else:
    ...
    try:
        ...
        while True:
            ...
            annotated_frame, current_loaded, current_unloaded, current_next_id, items_in_frame = process_single_frame(
                ...
                item_id_map, next_display_item_id, item_zone_tracking_info,
                loaded_count, unloaded_count, show_zone_border=False # MODIFIED: No border during processing
            )
...
def live_stream_processing_loop(selected_class, transfer_zone_rect, conf_threshold, camera_index):
    ...
    unique_metric_live_ph = live_metrics_col1.empty() 
    ...
    try:
        while st.session_state.get('live_stream_active', False): 
            ...
            annotated_frame, current_loaded, current_unloaded, current_next_id, _ = process_single_frame(
                frame, model, selected_class, transfer_zone_rect, conf_threshold,
                item_id_map, next_display_item_id, item_zone_tracking_info,
                loaded_count, unloaded_count, show_zone_border=False # MODIFIED: No border during live detection
            )
...
if st.session_state.processing_mode == "Upload Video":
    ...
    with col1:
        ...
        if st.session_state.first_frame_roi is not None:
            # For ROI setup, show_transparent=False to draw the purple box
    ...
    with col2:
        ...
        if st.button(f"🚀 Process Video for {st.session_state.selected_class}s", use_container_width=True, key="process_video_btn", disabled=process_button_disabled):
            # st.info(f"Processing video for '{st.session_state.selected_class}'... ROI will NOT be shown during processing.") # Message updated
            st.session_state.processed_video_path = None 
            ...
            for result in process_video_streamlit(video_bytes, st.session_state.selected_class, 
                ...
                final_results_from_processing = result
            ...
            if final_results_from_processing and final_results_from_processing[5] and os.path.exists(final_results_from_processing[5]):
                 st.session_state.processed_video_path = final_results_from_processing[5] 
                 try:
                    ...
                    if cap_thumb.isOpened():
                        ...
                        if ret_thumb:
                            # For thumbnail, show_transparent=True to draw the thin border
                            thumb_with_roi = draw_roi_preview(frame_thumb, transfer_zone_from_ui, show_transparent=True)
                            video_output_placeholder.image(cv2_to_pil(thumb_with_roi), caption="Processed Video (first frame with ROI border)", use_container_width=True)
                        cap_thumb.release()
    ...
    with col1_live:
        ...
        if model is None:
            ...
            if st.session_state.live_stream_state == "initial":
                ...
                st.info("Live stream is active. ROI will NOT be shown during detection.") # Message updated
                ...
                st.markdown(f"**ROI (x,y,w,h):** `{roi_disp}` (Not displayed on stream)")
...
with st.sidebar.expander("❓ Help & Tips"):
    ...
    **ROI Adjustment Tips (Upload Video & Live Stream):**
    - The transfer zone is a purple box during setup.
    - During active video processing or live detection, the ROI is **not** visually drawn on the frames.
    - The first frame thumbnail shown *after* video processing will display a thin border for the ROI used.
    
    **Live Stream Tips:**
    - Capture a frame from your camera to set up the ROI (purple box).
    - Ensure good lighting for better detection.
    - Test camera connection using the 'Camera Test' utility.
    
    **General:**
    - Adjust 'Detection Confidence Threshold' if items are missed or wrongly detected.
    - Ensure `best_bag_box.pt` model file is correctly placed.
    """)
