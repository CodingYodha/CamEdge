...
def process_single_frame(frame, model_instance, selected_class, transfer_zone_coords, conf_threshold, 
    ...
    frame_height, frame_width = frame.shape[:2] # Get current frame dimensions
    ...
    try:
        ...
        results = model_instance.track(frame, **track_args) # Use original frame for tracking
    except Exception as e:
        return annotated_frame, loaded_count[0], unloaded_count[0], next_display_item_id[0], current_item_count_frame
    
    # --- Dynamic Font Scaling Parameters ---
    REFERENCE_FRAME_WIDTH = 640  # Width for which the base font scale is defined
    BASE_FONT_SCALE_INFO = 0.8    # Base font scale for Loaded/Unloaded
    BASE_FONT_SCALE_ITEMS = 0.7   # Base font scale for "Items in frame"
    MIN_FONT_SCALE = 0.4
    MAX_FONT_SCALE = 1.2 # Prevent excessively large fonts
    ...
    # Dynamic BBox label font scale (optional, can also be scaled)
    ...
    if results[0].boxes is not None and len(results[0].boxes) > 0:
        ...
        for i in range(len(boxes_data)):
            ...
            # Apply dynamic scale to bbox labels too
    ...
    # --- Apply dynamic font scale to OSD text ---
    font_face = cv2.FONT_HERSHEY_SIMPLEX
    text_thickness = 2 # Usually, thickness doesn't need to scale as much as font size
    bg_color = (50,50,50)
    
    # Adjust y_offset and padding based on dynamic font scale to maintain good spacing
    # This is an approximation; you might need to fine-tune these factors
    base_y_offset_increment = 40
    base_padding = 5
    
    # Scale y_offset and padding
    # The scaling factor for these might be less aggressive than the font scale itself
    # For example, use sqrt(scale_factor) or a capped scale_factor
    ui_element_scale_factor = max(0.7, min(1.3, scale_factor)) # Cap the scaling for UI elements
    ...
    initial_y_pos = int(30 * ui_element_scale_factor) # Scale initial y position too

    y_offset = initial_y_pos
    
    draw_text_with_background(annotated_frame, f"Loaded: {loaded_count[0]}", 
                              (10, y_offset), font_face, dynamic_font_scale_info, 
                              (0,255,0), bg_color, text_thickness, dynamic_padding)
    y_offset += dynamic_y_offset_increment
    
    draw_text_with_background(annotated_frame, f"Unloaded: {unloaded_count[0]}", 
                              (10, y_offset), font_face, dynamic_font_scale_info, 
                              (0,0,255), bg_color, text_thickness, dynamic_padding)
    y_offset += dynamic_y_offset_increment
    
    draw_text_with_background(annotated_frame, f"Items in frame: {current_item_count_frame}", 
                              (10, y_offset), font_face, dynamic_font_scale_items, 
                              (0,255,255), bg_color, text_thickness, dynamic_padding)
