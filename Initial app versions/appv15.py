...
if model is not None and hasattr(model, 'model') and hasattr(model.model, 'names'):
    ...
    st.session_state.model_load_error = None 
elif model is None and not st.session_state.model_load_error: 
    st.session_state.model_load_error = "Model could not be loaded (unknown reason)."
...
def process_single_frame(frame, model_instance, selected_class, transfer_zone_coords, conf_threshold, 
                        item_id_map, # Removed next_display_item_id as it was for unique items
    ...
    try:
        ...
        results_list = model_instance.track(frame, **track_args) 
        
        if results_list: 
            detections_result_obj = results_list[0] 
            
    except Exception as e:
        return annotated_frame, loaded_count[0], unloaded_count[0], current_item_count_frame # Return items in frame instead of next_id
    
    REFERENCE_FRAME_WIDTH = 640; BASE_FONT_SCALE_INFO = 0.8; BASE_FONT_SCALE_ITEMS = 0.7
    MIN_FONT_SCALE = 0.4; MAX_FONT_SCALE = 1.2
    scale_factor = frame_width / REFERENCE_FRAME_WIDTH if REFERENCE_FRAME_WIDTH > 0 else 1.0
    ...
    if detections_result_obj and detections_result_obj.boxes is not None and len(detections_result_obj.boxes) > 0:
        boxes_data = detections_result_obj.boxes.xyxy.cpu().numpy()
        confs_data = detections_result_obj.boxes.conf.cpu().numpy()
        
        original_tracker_ids_data = None
        if hasattr(detections_result_obj.boxes, 'id') and detections_result_obj.boxes.id is not None:
            original_tracker_ids_data = detections_result_obj.boxes.id.cpu().numpy().astype(int)
        ...
        for i in range(len(boxes_data)):
            ...
            item_cls_id = None
            if hasattr(detections_result_obj.boxes, 'cls') and detections_result_obj.boxes.cls is not None and i < len(detections_result_obj.boxes.cls):
                item_cls_id_tensor = detections_result_obj.boxes.cls[i]
            ...
            # Removed unique item ID generation (next_display_item_id logic)
            # The original_tracker_id from ByteTrack is still used for zone tracking
            
            label = label_base # Default label if no tracker ID
            if original_tracker_ids_data is not None and i < len(original_tracker_ids_data): 
                original_tracker_id = original_tracker_ids_data[i] # This is the ID from ByteTrack
                label = f"ID:{original_tracker_id} {label_base}" # Display ByteTrack ID

                # Zone tracking logic based on ByteTrack ID
                currently_in_zone = is_centroid_in_zone(cx, cy, TRANSFER_ZONE_COORDS_x1y1x2y2)
                if original_tracker_id not in item_zone_tracking_info: # Use original_tracker_id as key
                    item_zone_tracking_info[original_tracker_id] = {"was_in_zone": currently_in_zone, "has_been_counted_load": False, "has_been_counted_unload": False}
                
                info = item_zone_tracking_info[original_tracker_id]
            ...
            if original_tracker_ids_data is not None and i < len(original_tracker_ids_data): # Only draw centroid if tracked
    ...
    # Return current_item_count_frame instead of next_display_item_id
    return annotated_frame, loaded_count[0], unloaded_count[0], current_item_count_frame

def process_video_streamlit(video_bytes, selected_class, transfer_zone_rect, conf_threshold, show_roi_on_video_toggle_val):
    if model is None: yield None, None, None, None, None; return # Removed one None for unique items
    ...
    try:
        ...
        item_id_map = {} # Still needed for ByteTrack IDs if used for zone logic
        item_zone_tracking_info = {}
        loaded_count = [0]
        unloaded_count = [0]

        cap = cv2.VideoCapture(temp_input_path)
        if not cap.isOpened(): yield None,None,None,None,None; return
        ...
        if not writer.isOpened(): yield None,None,None,None,None,output_video_path; return # Add output path for error
        frame_count = 0
        while True:
            ret,frame = cap.read()
            if not ret or frame is None: break
            frame_count +=1
            # Removed next_display_item_id from args and return values of process_single_frame
            annotated_frame,cur_loaded,cur_unloaded,items_in_frame = process_single_frame(
                frame, model, selected_class, transfer_zone_rect, conf_threshold, item_id_map,
            ...
            yield annotated_frame, cur_loaded, cur_unloaded, progress, None # Removed items_in_frame from yield for now as it's not a main metric like L/U
    except Exception as e:
        ...
        yield None, loaded_val, unloaded_val, None, output_video_path # Adjusted yield
    finally:
        if cap: cap.release()
    ...
    yield None, final_loaded, final_unloaded, 1.0, current_processed_path # Adjusted yield
...
def live_stream_processing_loop(selected_class, transfer_zone_rect, conf_threshold, source_identifier):
    ...
    item_id_map = {} # For ByteTrack IDs
    item_zone_info = {}
    loaded_ct = [0]
    unloaded_ct = [0]

    stream_ph = st.empty()
    # Removed unique_ph metric display
    live_met_col1,live_met_col2 = st.columns(2) # Adjusted to 2 columns
    loaded_ph = live_met_col1.empty()
    unloaded_ph = live_met_col2.empty()
    ...
    try:
        while st.session_state.get('live_stream_active', False):
            ...
            # Removed next_disp_id from args and items_fr from return
            annot_fr,cur_tot_load,cur_tot_unload, _ = process_single_frame( # _ for items_in_frame
                frame, model, selected_class, transfer_zone_rect, conf_threshold,
                item_id_map, item_zone_info, loaded_ct, unloaded_ct, 
                show_roi_on_video_frames=show_roi_live
            )
            stream_ph.image(cv2_to_pil(annot_fr), channels="RGB", use_container_width=True)
            
            # Removed unique items display
        ...
        stream_ph.empty(); loaded_ph.empty(); unloaded_ph.empty() # Removed unique_ph.empty()
...
default_states = {
    ...
    'final_loaded_count': 0, 'final_unloaded_count': 0, # Removed final_unique_count
...
if st.session_state.processing_mode != new_processing_mode: 
    ...
    if new_processing_mode == "Upload Video":
        ...
        st.session_state.final_loaded_count = 0; st.session_state.final_unloaded_count = 0 # Removed final_unique_count
    st.rerun()
...
if st.session_state.processing_mode == "Upload Video":
    ...
    with col2:
        ...
        st.markdown("### 📈 Metrics")
        # Adjusted to 2 columns for metrics
        m_col1, m_col2 = st.columns(2)
        loaded_metric_ph = m_col1.empty()
        unloaded_metric_ph = m_col2.empty()
        # Removed unique_metric_ph 
        progress_placeholder = st.empty()
        status_message_placeholder = st.empty()

        if model is None:
            status_message_placeholder.error("🚨 Model not loaded! Processing is disabled.")
            if 'video_display_placeholder' in locals(): video_display_placeholder.warning("🖼️ Video processing requires a loaded model.")
            ...
            if 'video_display_placeholder' in locals(): video_display_placeholder.info("🖼️ Video output will appear here after processing.")
        ...
        if st.session_state.is_processing:
            ...
            unloaded_metric_ph.metric("Total Unloaded", st.session_state.final_unloaded_count)
            # Removed unique items display
            if st.session_state.processing_status_message and model and uploaded_file: 
                 status_message_placeholder.info(st.session_state.processing_status_message)
        ...
        if st.button(f"🚀 Process Video for {st.session_state.selected_class}s", use_container_width=True, key="process_video_btn", disabled=process_btn_disabled):
            ...
            st.session_state.processed_video_path = None; st.session_state.final_loaded_count=0; st.session_state.final_unloaded_count=0 # Removed unique
            ...
            show_roi_toggle_val = st.session_state.show_roi_on_video_toggle_upload 
            progress_bar = progress_placeholder.progress(0); final_res_gen = None
            for frame_data in process_video_streamlit(video_bytes,st.session_state.selected_class,transfer_zone,conf_threshold_st,show_roi_toggle_val):
                # Adjusted unpacking of frame_data
                annot_fr,load,unload,prog,tmp_vid_path = frame_data; final_res_gen = frame_data
                if annot_fr is not None: video_display_placeholder.image(cv2_to_pil(annot_fr), caption="Processing...", use_container_width=True)
                if load is not None: 
                    loaded_metric_ph.metric("Loaded",load)
                    unloaded_metric_ph.metric("Unloaded",unload)
                    # Removed unique metric update
                if prog is not None: progress_bar.progress(prog)
            st.session_state.is_processing = False
            if final_res_gen:
                # Adjusted unpacking
                _,fin_load,fin_unload,_,fin_path = final_res_gen 
            ...
            if st.session_state.live_stream_state == "initial":
                ...
                if st.session_state.live_stream_captured_frame is not None:
                    ...
                    st.toggle("Show ROI on Live Stream", 
                              ...
                              help="If ON, the transfer zone will be drawn on the live feed.")
                ...
                st.toggle("Show ROI on Live Stream", 
                          key="show_roi_on_live_stream_toggle", 
                          help="Toggle ROI visibility on the live feed.")
...
with st.sidebar.expander("❓ Help & Tips", expanded=False):
    ...
    **Live Stream/Webcam Mode:**
    ...
    - "Show ROI on Live Stream" toggle (`key="show_roi_on_live_stream_toggle"`) controls visibility on the live feed.
